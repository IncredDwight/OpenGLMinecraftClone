...
static float grad(int32_t hash, float x) {
    ...
    float grad = 0.5f + (h & 7);    // Gradient value 1.0, 2.0, ..., 8.0
...
float SimplexNoise::noise(float x, float y) {
    ...
    const float x2 = x0 - 0.5f + 2.0f * G2;   // Offsets for last corner in (x,y) unskewed coords
    const float y2 = y0 - 0.5f + 2.0f * G2;
...
float SimplexNoise::noise(float x, float y, float z) {
    ...
    static const float F3 = 0.5f / 3.0f;
    static const float G3 = 0.5f / 6.0f;
    ...
    float x3 = x0 - 0.5f + 3.0f * G3; // Offsets for last corner in (x,y,z) coords
    float y3 = y0 - 0.5f + 3.0f * G3;
    float z3 = z0 - 0.5f + 3.0f * G3;
