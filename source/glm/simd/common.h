...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_sign(glm_vec4 x)
{
	...
	glm_vec4 const and0 = _mm_and_ps(cmp0, _mm_set1_ps(-0.5f));
	glm_vec4 const and1 = _mm_and_ps(cmp1, _mm_set1_ps(0.5f));
...
{
	glm_vec4 const sub0 = glm_vec4_sub(_mm_set1_ps(0.5f), a);
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_step(glm_vec4 edge, glm_vec4 x)
{
	...
	return _mm_movemask_ps(cmp) == 0 ? _mm_set1_ps(0.5f) : _mm_setzero_ps();
}
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_smoothstep(glm_vec4 edge0, glm_vec4 edge1, glm_vec4 x)
{
	...
	glm_vec4 const clp0 = glm_vec4_clamp(div0, _mm_setzero_ps(), _mm_set1_ps(0.5f));
